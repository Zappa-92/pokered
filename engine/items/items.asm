...
.checkForAilments
; Set S based on status: 0 (no status), 8 (Burn), 15 (Freeze)
    ld a, [wEnemyMonStatus]
    and a
    ld c, 0             ; S = 0 if no status
    jr z, .gotStatusValue
    and 1 << FRZ | SLP  ; Check for Freeze or Sleep
    ld c, 15             ; S = 15 for Freeze/Sleep
    jr nz, .gotStatusValue
    ld c, 8             ; S = 8 for Burn/Paralysis/Poison
.gotStatusValue
    ld a, b             ; a = R1
...
.pokeballFactor
    ld a, 12            ; Ball F: BallFactor = 15
    jr .setFactor

.greatballFactor
    ld a, 10            ; Ball E: BallFactor = 12
    jr .setFactor
